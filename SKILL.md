...
8. **Literature crowding gate for thesis topics**: for master's students, thesis rescue mode, and crowded China topics such as digitalization, common prosperity, digital finance, green finance, smart cities, low-carbon pilots, ESG, high-quality development, rural revitalization, and new quality productive forces, search Chinese and English literature before narrowing the title. First distinguish a known broad idea from an exhausted concrete design. Do not kill merely because the broad intuition appears in prior work; classify the contribution tier and continue if the candidate has a real increment in setting, measurement, identification, magnitude, channels, boundary conditions, or external validity. If the closest papers already exhaust the concrete X, Y, data, setting, method, mechanism, and contribution boundary, say so and recommend `park`, `kill`, or a major pivot. See `references/07_literature_crowding_gate.md`.
...
- **Crowded-literature gate**: for master's thesis ideas in crowded literatures, do not proceed to title polishing or a full blueprint until closest Chinese and English papers have been checked. Classify both the crowding level and the contribution tier: `frontier despite known idea`, `defensible thesis extension`, `downgrade only`, or `exhausted / kill`. If the broad idea is known but the candidate has a hard increment, do not kill by novelty alone. If the idea is only an old concrete X-Y-data-method-mechanism package with a standard index, standard panel FE/DID, and no new data, setting, measurement, identification, magnitude, channel, boundary, or contribution increment, recommend stopping, changing topic, or downgrading.
...
0e. **文献拥挤度 gate**: low / medium / high / saturated, repeat object, contribution tier, closest literature pattern, defense risk, and whether to continue.
...
- `references/02_verdict_rules.md`: green/yellow/red feasibility, contribution tier, and proceed/pivot/park/kill/upgrade/downgrade decisions.
...
- `references/07_literature_crowding_gate.md`: Chinese/English literature search-first rule, known-idea versus exhausted-design distinction, contribution tier, crowded-topic stop rules, and ways to find remaining thesis space.
